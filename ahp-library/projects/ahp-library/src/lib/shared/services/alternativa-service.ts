...
export class AlternativaService {
  ...
  deleteAlternativa(i: number) {
    this._listaAlternativa.update((trenutnaLista) =>
      trenutnaLista.filter((_, index) => index !== i),
    );
  }
  ...
  listaAlternativaSaVrijednostima = computed<Alternativa[]>(() => {
    ...
    for (let i = 0; i < n; i++) {
      let prosjecnaVrijednostAltenative = 0;
      for (let j = 0; j < m; j++) {
        ...
        if (kriterijum) {
          ...
          kriterijum.value =
            this.tezinskiVektor[j] * (kriterijKoristiPom ? currentValue : reciprocnaValue);
          prosjecnaVrijednostAltenative += kriterijum.value;
        }
      }
      lista[i].value = prosjecnaVrijednostAltenative;
    }
    return lista;
  });
}
